<template>
  <div class="modal" :class="this.verifyModal.modalOptions.type">
    <div class="modal-wrapper">
      ...
      <div class="modal-content" v-html="this.verifyModal.modalOptions.content"></div>
	...
	methods: {
		async handleClose() {
			if (this.verifyModal.actionID === "Namespace") {
				...
				await this.$store.dispatch(
					"name/getN",
					`${this.verifyModal.server.initialMeta}${this.verifyModal.parentNamespace}/names${this.verifyModal.server.maxLimit}`
				);
			}
		...
		async handleDelete() {
			if (this.verifyModal.actionID === "Namespace") {
				...
				await this.$store.dispatch(
					"name/getN",
					`${this.verifyModal.server.initialMeta}${this.verifyModal.parentNamespace}/names`
				);
			}
		}
	},
